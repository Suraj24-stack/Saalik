    ...
    <div className="relative h-screen bg-gradient-to-br from-black via-gray-900 to-green-900">
      ...
      <div className="relative h-full flex items-center justify-center px-4">
        ...
        <div className="text-center max-w-5xl">
          <p className="text-green-400 text-lg md:text-xl mb-6 tracking-wide">{slides[currentSlide].subtitle}</p>
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold text-white mb-2">{slides[currentSlide].title}</h1>
          <h2 className="text-5xl md:text-7xl lg:text-8xl font-bold text-green-400">{slides[currentSlide].highlight}</h2>
        </div>
      ...
      <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 flex space-x-3 z-10">
        {slides.map((_, index) => (
          <button key={index} onClick={() => setCurrentSlide(index)}
            className={`h-2 rounded-full transition-all ${index === currentSlide ? 'bg-green-400 w-12' : 'bg-white/40 w-2 hover:bg-white/60'}`} />
        ))}
      </div>
    </div>
  );
