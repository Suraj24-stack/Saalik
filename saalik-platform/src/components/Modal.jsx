      ...
      <div className="bg-gray-900 rounded-2xl p-8 max-w-md w-full border-2 border-green-500/30">
        ...
        <div className="space-y-4">
          ...
          <input type="email" placeholder="Your Email" required value={formData.email}
            ...
            className="w-full px-4 py-3 bg-black border border-green-500/30 rounded-lg text-white focus:border-green-400 outline-none" />
          <textarea placeholder="Message (Optional)" rows={4} value={formData.message}
          ...
          <button onClick={handleSubmit} className="w-full bg-green-500 hover:bg-green-400 text-black font-bold py-3 rounded-lg transition-all">
            Join Waitlist
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl p-8 max-w-md w-full border-2 border-green-500/30">
        ...
        <div className="space-y-4">
          <input type="text" placeholder="Story Title" required value={formData.title}
            onChange={(e) => setFormData({...formData, title: e.target.value})}
            className="w-full px-4 py-3 bg-black border border-green-500/30 rounded-lg text-white focus:border-green-400 outline-none" />
          <input type="text" placeholder="Location" required value={formData.location}
            onChange={(e) => setFormData({...formData, location: e.target.value})}
            className="w-full px-4 py-3 bg-black border border-green-500/30 rounded-lg text-white focus:border-green-400 outline-none" />
          <textarea placeholder="Description" rows={5} required value={formData.description}
