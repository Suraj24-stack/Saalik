...
const PartnersSection = () => {
  ...
  return (
    <section className="relative py-20 md:py-28 overflow-hidden" style={{ backgroundColor: '#001410' }}>
      {/* Background Pattern */}
      ...
      {/* Container with 2.5% left & right padding */}
      <div className="w-full px-[2.5%] relative">
        ...
        {error && !loading && (
          <div className="mb-20 md:mb-28 max-w-2xl mx-auto">
            <div className="text-center py-8 px-6 rounded-2xl border-2"
                 style={{ background: 'rgba(239, 68, 68, 0.1)', borderColor: 'rgba(239, 68, 68, 0.4)' }}>
              <svg className="mx-auto h-12 w-12 mb-4" style={{ color: '#ef4444' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              return (
                <div key={partner.id} className="flex flex-col items-center text-center group"
                  ...
                  <div className="relative w-28 h-28 md:w-32 md:h-32 lg:w-36 lg:h-36 mb-4 md:mb-6">
                    <div className={`absolute inset-0 bg-green-400/20 rounded-full blur-2xl scale-110 transition-opacity duration-300 ${hoveredPartner === partner.id ? 'opacity-100' : 'opacity-0'}`} />
                    
                    <div className={`relative w-full h-full rounded-full bg-gradient-to-br from-gray-900 to-black flex items-center justify-center transition-all duration-300 shadow-xl cursor-pointer`}
                         style={{
                           border: hoveredPartner === partner.id ? '2px solid #10b981' : '2px solid rgba(16, 185, 129, 0.4)',
                           boxShadow: hoveredPartner === partner.id ? '0 0 30px rgba(16, 185, 129, 0.3)' : '0 0 0 rgba(0,0,0,0)'
                         }}
                         onClick={() => {
                           if (partner.website) window.open(partner.website, '_blank', 'noopener,noreferrer');
                         }}>
                      {fullLogoUrl ? (
                        <img src={fullLogoUrl} alt={partner.name} className="w-20 h-20 md:w-24 md:h-24 object-contain p-2"
                             style={{ filter: hoveredPartner === partner.id ? 'brightness(1.1)' : 'brightness(1)', transition: 'filter 0.3s ease' }}
                      ...
                      {partner.website && hoveredPartner === partner.id && (
                        <div className="absolute inset-0 rounded-full flex items-center justify-center"
                          ...
                          <span className="text-xs font-bold px-3 py-1 rounded-full" style={{ background: '#10b981', color: '#000' }}>Visit</span>
                        </div>
                      )}
                    </div>
                  </div>

                  <p className="text-green-400 text-sm md:text-base font-semibold leading-tight max-w-[180px]">{partner.name}</p>
                  {partner.description && hoveredPartner === partner.id && (
                    <p className="text-gray-400 text-xs mt-2 max-w-[180px] leading-relaxed"
                       style={{ display: '-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden', animation:'fadeIn 0.3s ease-in' }}>
                      {partner.description}
                    </p>
                  )}
                </div>
